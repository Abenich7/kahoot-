...
class RequestHandler(Thread):
    ...
    def process_request(self, data):
        ...
        if parts[0] == "login":
            ...
            json.dump(users,"users.json")
            return "Signup successful"
          
        elif parts[0] == "get_quizzes":
            quizzes_names = json.dumps(quizzes_names)
        ...
        # Store the username as the admin for the game ID (e.g., using the game name)
            ...
            game_srv=GameServer(username,game_id)

            return(str(game_id).encode())
        
        elif parts[0] == "join_game":
            game_id = parts[1]
            # Handle join game request
            if game_id == game_srv.game_id:
                return "success"
            else: 
                return "No"
...
class GameServer:
    def __init__(self,game_admin,game_id):
        self.game_admin=game_admin
        self.game_id=game_id

     #  self.game_events = {}
     #   self.admins = {}  # Dictionary to store admins for each game ID
...
if __name__ == "__main__":
    ...
    try:
        with open("game_events.json", "r") as file:
            game_events = json.load(file)
    except FileNotFoundError:
    # If file doesn't exist, create one with initial value
        with open("game_events.json", "w") as file:
            json.dump(game_events, file)

    try:
        with open("game_admins.json", "r") as file:
            game_admins = json.load(file)
    except FileNotFoundError:
    # If file doesn't exist, create one with initial value
        with open("game_admins.json", "w") as file:
            json.dump(game_admins, file)
   ...
   # game = KahootGame()
    ...
    game_server = GameServer(game_admins,game_events)
    # Start the server
